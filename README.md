...
Для входа используется отдельная пара `DB_LOGIN_USER`/`DB_LOGIN_PASSWORD`:
ей нужен только `EXECUTE` на процедуру `decanet.GETRUINFO`. В тестовой базе
по умолчанию это `guest`/`guest`; после успешного входа приложение подключается
как пользователь, назначенный учётной записи Decanet.
...
Новые функции размещайте в типизированных слоях `Application`, `Http`,
`Infrastructure`, `Repository` и `Security`. Legacy-страницы в `src/` временно
подключаются только через маршрутизатор и должны мигрировать модуль за модулем.
Учётные данные подключения задаются исключительно в `.env`; не храните их в
сессиях или коде. Публичные адреса используют расширение `.php`.
